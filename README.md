...
```bash
git clone https://github.com/tu-usuario/bitacora-proyectos.git
cd bitacora-proyectos
pnpm install
# o: npm install
```
...
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | URL del proyecto de Supabase (Project Settings → API). |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Clave pública (anon) del proyecto. Es segura de exponer en el cliente porque el acceso está limitado por las políticas RLS de solo lectura. |
...
3. En **Environment Variables**, agrega `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` con los mismos valores de tu `.env.local`.
